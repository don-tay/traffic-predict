...
names_rows = [
    ...
    "humidity_realtime",
    "air_temp_realtime",
    "4day_temperature_low_1",
    "4day_temperature_low_2",
    "4day_temperature_low_3",
    "4day_temperature_low_4",
    "4day_temperature_high_1",
    "4day_temperature_high_2",
    "4day_temperature_high_3",
    "4day_temperature_high_4",
    "4day_relative_humidity_low_1",
    "4day_relative_humidity_low_2",
    "4day_relative_humidity_low_3",
    "4day_relative_humidity_low_4",
    "4day_relative_humidity_high_1",
    "4day_relative_humidity_high_2",
    "4day_relative_humidity_high_3",
    "4day_relative_humidity_high_4",
    "4day_wind_speed_low_1",
    "4day_wind_speed_low_2",
    "4day_wind_speed_low_3",
    "4day_wind_speed_low_4",
    "4day_wind_speed_high_1",
    "4day_wind_speed_high_2",
    "4day_wind_speed_high_3",
    "4day_wind_speed_high_4",
    "24hr_general_relative_humidity_low",
    "24hr_general_relative_humidity_high",
    "24hr_general_temperature_low",
    "24hr_general_temperature_high",
    "24hr_general_wind_speed_low",
    "24hr_general_wind_speed_high",
]
...
cur3.execute(
    "SELECT DISTINCT call_timestamp, SUBSTRING(call_timestamp, 1, 10) AS date,\
...
class CongestionDisplayApp(server.App):
    ...
    controls = [
        {"type": "button", "label": "Update", "id": "update_all"},
        {"type": "button", "label": "Change Image", "id": "pic_change"},
        {"type": "button", "label": "Predict", "id": "make_predict"},
    ]
    ...
    def get_image(self, params):
        ...
        narrow = all_time_df[
            (all_time_df.date == image_date) & (all_time_df.hour == image_time[0:2])
        ].copy()
    ...
    def analysis_table(self, params):
        ...
        if full_end < full_start:
            return pd.DataFrame(
                ["End is greater than start. Please change the time period"]
            )
    ...
    def analysis_plot(self, params):
        ...
        get_values = (
            get_values
            + str(camera_id)
            + f" AND call_timestamp >= {full_start} AND call_timestamp <= {full_end} ORDER BY date, time"
        )
        ...
        plot_df = pd.DataFrame(
            data2, columns=["call", "direction", "date", "time", "congestion"]
        )
        plot_df["numeric"] = plot_df.apply(
            lambda x: congestion_map[x.congestion], axis=1
        )
        ...
        for label, data in plot_df.groupby("direction"):
            ...
            ax[label - 1].set(
                title=f"Congestion against time for direction {label}",
                ylabel="Congestion",
                xlabel="Time",
            )
