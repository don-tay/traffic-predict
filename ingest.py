...
def get_req_handler(url: str) -> requests.Response:
    ...
    if response.status_code != 200:
        print("Non-OK status " + str(response.status_code) +
...
response = get_req_handler(os.environ["IMAGE_API_URL"])
...
for camera in cameras:
    image, location, camera_id, image_metadata = itemgetter(
        'image', 'location', 'camera_id', 'image_metadata')(camera)
    # sample filename format: cam-img/2022-03-21T235548_1002.jpg
    file_name = os.environ["CAM_IMG_DIR"] + iso_datetime + \
        '_' + camera_id + '.jpg'
    print(file_name)
    # get image stream
